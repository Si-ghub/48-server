...
server.httpServer = http.createServer((req, res) => {
    ...
    // rezervuota atminties vieta-buffer
    let buffer = '';
    req.on('data', (data) => {
    ...
    req.on('end', async () => {
        ...
        if (isTextFile || isBinaryFile) {
            ...
            const html = await handler(data); // iskvieti funkcija kuria gavome handleryje. Turi grazinti turini, t.b su await (visada jei yra async)
            if (html === '') {
                res.writeHead(404, {
